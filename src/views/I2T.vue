    ...
    <v-row>
      <v-col cols="6">
        <v-card height="500" max-height="500">
          <input type="file" accept="image/*" @change="onImageUpload" value="upload your img!" class="submit-btn" style="justify-self: center">
          <br/>
      ...
      <v-col cols="6">
        <v-card height="500">
          <v-card-title>Dense Caption of IMG</v-card-title>
          <v-card-text>
            <div>{{ caption }}</div>
            <div>{{ imageSize }}</div>
            <div>{{ imageType }}</div>
